...
def predict():
    input_values = [0 if k == "gender" and v.lower() == "male" else 1 if k == "gender" and v.lower() == "female" else float(v) for k, v in request.json.items() if k not in ["Name", "Nickname", "Gender", "Date_of_birth", "Weight", "Height", "Team", "Shirt_number", "Position"]]
...
def get_data():
    ...
    all_players_df = pd.concat([goalkeepers_file, midfielders_file, forwards_file, defenders_file])
...
def get_altdata():
    ...
    goalkeepers_df = goalkeepers_file.sort_values(by='score', ascending=False)
    midfielders_df = midfielders_file.sort_values(by='score', ascending=False)
    forwards_df = forwards_file.sort_values(by='score', ascending=False)
    defenders_df = defenders_file.sort_values(by='score', ascending=False)
...
def get_comparison():
    players = []
    files = ['defenders.csv', 'forwards.csv', 'goalkeepers.csv', 'mmidfielders.csv']
    for file in files:
        df = pd.read_csv(file)
